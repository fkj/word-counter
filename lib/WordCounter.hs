module WordCounter (module WordCounter) where
...
main = do
  args <- getArgs
  case args of
    [] -> T.IO.putStrLn (T.pack "Expected a file name.") >> exitFailure
    [filename] -> do
      ...
      case textOrError of
        ...
        Right text ->
          T.IO.putStr $ T.unlines $ map formatCount $ processText text
    _:_:_ -> T.IO.putStrLn (T.pack "Expected only one argument: a file name.") >> exitFailure

processText :: T.Text -> [(T.Text, Int)]
processText text = countAndSort $ T.words $ T.toCaseFold text
